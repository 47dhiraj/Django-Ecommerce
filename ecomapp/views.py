...
class EcomMixin(object):                                                
    ...
    def dispatch(self, request, *args, **kwargs):                       
        ...
        if session_cart_id:                                                     
            cart_obj = Cart.objects.get(id=session_cart_id)                     
            
            if request.user.is_authenticated and request.user.customer: 
                try:
                    previous_cart_obj = Cart.objects.get(customer=self.request.user.customer)
...
class AddToCartView(EcomMixin, TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        # get product id coming from url parameter
        product_id = self.kwargs['pro_id']

        # For getting product running the query
        ...
        if self.request.user.is_authenticated: 
            try:
                ...
                if this_product_in_cart.exists():                            
                    ...
                    cartproduct = CartProduct.objects.create(cart=cart_obj, product=product_obj, rate=product_obj.selling_price, quantity=1, subtotal=product_obj.selling_price)         # cartproduct object create gareko
                    cart_obj.total += product_obj.selling_price
                    cart_obj.save()
            except:
